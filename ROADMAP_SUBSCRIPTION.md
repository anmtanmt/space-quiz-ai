...
- [x] **Task 5: 法務表記・利用規約ページの設置** (完了)
  - 特定商取引法に基づく表記（販売業者、責任者、所在地、販売価格、解約方法など）。
  - 利用規約 & プライバシーポリシー（3タブ切り替えモーダル & React Portal実装）。
  - Stripe 審査対応の直リンク用URLルーティング（`?page=tokusho`, `?page=terms`, `?page=privacy`）完備。
  - 親ポータルおよびタイトル画面フッターからのワンタップ閲覧に対応。
- [ ] **Task 6: Stripe 本番化**
