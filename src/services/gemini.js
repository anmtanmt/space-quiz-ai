...
const FALLBACK_QUIZZES = {
  easy: [
    ...
    },
    {
      ...
      answerIndex: 1,
      explanation: 'ロケットは、ちきゅうの じゅうりょくを ふりきって うちゅうへ いくために、ものすごい スピードで とぶ のりものだよ。'
    },
    {
      id: 'fb_e5',
    ...
    {
      id: 'fb_e8',
      question: 'つきには、いきをするための くうきは あるかな？',
...
// 天文宇宙検定用のクイズ生成API
export const generateAstronomyTestQuiz = async (grade, answeredIds = [], excludeQuestions = [], recentQuestions = [], targetChapter = null, targetImage = null) => {
  const apiGatewayUrl = getApiGatewayUrl();
  ...
  if (apiGatewayUrl) {
    try {
      const response = await fetch(apiGatewayUrl, {
        ...
        body: JSON.stringify({ isTest: true, grade, answeredIds, excludeQuestions, recentQuestions, targetChapter, targetImage })
      });
...
// 検定クイズ用バリデーション
const validateTestQuiz = (quiz, targetImage = null) => {
  if (!quiz) return false;
  if (typeof quiz.question !== 'string' || quiz.question.length < 5) return false;
